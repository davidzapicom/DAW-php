...
<body>
	...
	if (isset($_POST['cerrarsesion'])) {
		...
		if ($_SESSION["rol"] === "consultor") {
			...
			foreach ($_SESSION["cesta"] as $indice => $producto) {
				...
				// Comprobar que la compra de cada producto se ha realizado correctamente
				$consultarCompra = "SELECT * FROM `compras`
				WHERE
					(id_usuario = '{$_SESSION["id_usuario"]}') AND
					(id_articulo = '{$producto[0]}') AND
					(fecha = '{$horaActual}') AND
					(cantidad = '{$producto[3]}') AND
					(precio_unitario = '{$producto[2]}')
				";
				$consultaCompra = mysqli_query($_SESSION["connection"], $consultarCompra);
				if (mysqli_num_rows($consultaCompra) !== 0) {
					$compraCorrecta = true;
				} else {
					$compraCorrecta = false;
				}
			...
			if ($compraCorrecta) {
				$compraSuccess = $_SESSION["usuario"] . ", su compra se ha gestionado correctamente.";
				// Vaciar el carrito despues de comprar satisfactoriamente
				$_SESSION["cesta"] = array();
			} else {
				$compraErr = "Ha ocurrido un error en la compra.";
			}
		}
	}
	echo '
	...
	if ($vacio) {
		if ($compraSuccess !== "") {
			echo '<p class="success">' . $compraSuccess . '</p>';
		}
		echo '
					<p>Carrito vacio</p>
		';
		...
		';
		if ($compraErr !== "") {
			echo '<p class="error">' . $compraErr . '</p>';
		}
	}
	echo '
				</div>
