...
<body>
	...
	if (isset($_POST['cerrarsesion'])) {
		...
		if ($consultaComprasErr === "") {
			$selectCompras = 'SELECT compras.fecha, articulos.descripcion, compras.precio_unitario, compras.cantidad FROM `compras`, `articulos` WHERE (fecha BETWEEN "' . $desde . '" AND "' . $hasta . '") AND (id_usuario = "' . $_SESSION["id_usuario"] . '") AND (compras.id_articulo = articulos.id_articulo) ORDER BY compras.fecha DESC';
